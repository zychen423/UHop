...
#                print(model_name_list, 'error', 'continue')
            ...
            for i in range(0, len(requirements), 2):
                ...
                result_list.append((save_model_dir, test_acc, test_loss, rc, td, [args['hidden_size'], args['dropout_rate'], args['l2_norm'], args['margin'], args['learning_rate']], args['dataset']))
result_list = sorted(result_list, key=lambda x: x[1])#(x[3][3], x[3][2], x[1]))
temp=[]
for result in result_list[-1:]:
    test_acc, test_loss, rc, td = result[1], result[2], result[3], result[4]
